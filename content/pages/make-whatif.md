...
1. Устонавливаем пакет html2text с помощью менеджера пакетов дистрибутива.
...
3. В полученном файле нужно:
    ...
    * Оформить текст с картинок, как описано далее в этом разделе. (*См. подраздел «Оформление текста с изображений».*)
    * Разместить текст сносок непосредственно после тех абзацев, где они эти сноски встречаются.
4. Создаём новую «главу» в [«книге» переводов What If?](http://notabenoid.com/book/41531), импортируем в неё содержимое `029-spent-fuel-pool.md`, тип переносов строк: `Два переноса`.
...
Для перевода просто помещаем надписи с картинки в `[labels]` и `[/labels]`. Выглядит это так:
...
* Формулы оборачиваются в `$` (внутристрочные) или в `$$` (на отдельной строке): `$F_i(x)$`, `$$F_i(x) = i*log_2(x)$$`.
