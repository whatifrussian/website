...
1. Мы публикуем перевод статьи в четверг (до 23:59 по московскому времени) или, если Рэндал задержался с публикацей, в понедельник.
...
3. Когда перевод надписей на картинках устоялся, нужно его нарисовать (обычно мы занимаемся этим в день выпуска). Договариваемся о том, кто рисует, через групчат. (*См. раздел «Отрисовка изображений».*)
...
1. Установить пакет html2text с помощью менеджера пакетов вашего дистрибутива.

### Конвертация и создание «главы» на Нотабеноиде

1. Открываем исходный текст последней статьи What If? (Ctrl+U) и сохраняем всё, что между тегами `<article>` и `</article>` в файл вида `029-spent-fuel-pool.html` (`029` — номер записи на сайте whatif, `spent-fuel-pool` — заголовок записи в нижнем регистре, пробелы заменяются на `-`).
2. Конвертируем в markdown командой: `html2text -b0 029-spent-fuel-pool.html > 029-spent-fuel-pool.md`, или через StackEdit: меню `Open from` → `Convert HTML to Markdown`, вставляем текст между `<atricle>` и `</article>`, сохраняем как `029-spent-fuel-pool.md`.
3. В полученном файле нужно:
    * Добавить «пустую» строку с символом `>` перед указанием автора вопроса.
    * Изменить базу url’ов картинок на `/uploads/029-spent-fuel-pool/` (например, /imgs/a/51/freefall_candy.png → /uploads/051-free-fall/freefall_candy_ru.png).
    * Оформить текст с картинок, как описано далее в этом разделе.
    * Разместить текст сносок непосредственно после тех абзацев, где они эти сноски встречаются.
4. В [«книге» переводов What If?](http://notabenoid.com/book/41531) создаём новую «главу», в неё импортируем содержимое `029-spent-fuel-pool.md`, тип переносов строк: **Два переноса**.
...
```
значит нужно удалить «подозрительные» символы, обычно это тире или кавычки. Под Linux такой проблемы не возникает. (Может, Python3 решит проблему?)

### Оформление текста с изображений

Для перевода просто помещаем надписи с картинки в [labels][/labels]. Выглядит это так:
...
* Ссылоки: `[текст ссылки](http://example.com)` - ссылки с русскими символами лучше именно с этими чимволами и вставлять, например, `http://ru.wikipedia.org/wiki/Я`, а не `http://ru.wikipedia.org/wiki/%D0%AF`. Если ссылка на запись на сайте, то она должна выглядеть так: `/page/pennies`.
* Изображения: `![](http://example.com/image.png) "alt-текст изображения"`) - локальная ссылка на картинку обычно выглядит так: /uploads/id-slug/original_name.png. Про то, как загружать изображения будет позже
* Сноски:
```
...
* Текст типа [citation needed] делается с использованием тега sup:
  * Если текст без ссылки: `<sup>[</sup><sup>источник?</sup><sup>]</sup>`
  * Если текст со ссылкой: `<sup>[[источник?](http://example.com)]</sup>`
...
В ходе работы у нас выработались определённые рекомендации для переводчиков. И выработалось их немало, поэтому теперь они живут на [отдельной странице](/translations-style/).
...
2. Интерфейс у GIMP не очень простой, но необходимые нам операции — несложные:
    1. Сперва нам нужно удалить текст. Сделать это очень просто, выделяем его и жмём `Delete`.
    2. Выбираем инструмент «Текст», выбираем шрифт и размер и пишем перевод. Позиционируем его чтобы смотрелось нормально.
    3. Дальше нужно выбрать снова нужный рабочий слой: `Ctrl-L` и выбираем самый нижний слой. Теперь снова можно выделять кусок картинки и очищать его.
...
2. Нужно зарегистрироваться на github.com (сгенерировать ключ и т. д.) и убедиться, что вы включены в организацию (whatifrussian)[https://github.com/whatifrussian].
...
2. `fab build`, чтобы убедиться, что всё необходимое (кроме rsync) установлено и работает.
3. `git pull`, чтобы убедиться, что локальный репозиторий синхронизирован с основным.
4. Добавить (и закоммитить) новый материал через git (любым удобным способом): статью и картинки.
5. Если материал добавлен в основной репозиторий, но его ещё нет в локальном, выполнить `git pull`.
6. fab publish:dev
7. Открыть [http://dev.chtoes.li/](http://dev.chtoes.li/) и убедиться, что всё отображается корректно.
8. Если материал добавлялся локально, но его ещё нет в основном репозитории, выполнить `git push`.
9. fab publish:prod
10. Открыть [http://chtoes.li](http://chtoes.li) и убедиться, что всё отображается корректно.

При проверке статьи в dev-версии сайта нужно обратить внимание, что отображаются переведённые картинки, а не оригиналы (для изображений с текстом). Если отображаются оригинальные изображения, нужно убедиться, что переведённые картинки загружены под именами, заканчивающимися на `_ru.png` и соответственно скорректировать url’ы в статье (добавить `_ru` перед `.png`).
