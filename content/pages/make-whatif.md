...
* Ссылки: `[текст ссылки](http://example.com)`. Ссылки должны оставаться человекочитаемыми: сокращать их и кодировать в %xx-нотацию не нужно (`http://ru.wikipedia.org/wiki/Я` предпочтительней `http://ru.wikipedia.org/wiki/%D0%AF`). Если ссылка ведет на страницу нашего сайта, базовую часть лучше опустить: `/page-name-here`. Для внешних ссылок желательно указать заголовок страницы: `[текст ссылки](http://example.com "заголовок (англ.)")`. Для англоязычных материалов (за исключением видеороликов) заголовок приводится в переводе, но с указанием на язык страницы.
...
* Сноски:
```
Lorem ipsum[^1][^2] dolor sit amet, consectetur adipisicing elit
...
5. Собрать: `publish:dev`, проверить: [http://dev.chtoes.li](http://dev.chtoes.li); поправить недочеты и повторить.
...
7. Опубликовать на сайте: `fab publish:prod`, проверить: [https://chtoes.li](https://chtoes.li).
